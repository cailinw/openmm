...
static int getDataId(ContextImpl& context) {
	EmuPlatform::PlatformData* data = reinterpret_cast<EmuPlatform::PlatformData*>(context.getPlatformData());
    return data->dataId;
}
...
void EmuUpdateStateDataKernel::getEnergyParameterDerivatives(ContextImpl& context, <string, double>& derivs) {
	int dataId = getDataId(context)
